...
for line in fh:
    ...
    address = line.strip()
    print('')
    cur.execute("SELECT geodata FROM Locations WHERE address= ?", (memoryview(address.encode('utf-8')), ))
    ...
    cur.execute('''INSERT INTO Locations (address, geodata)
            VALUES ( ?, ? )''', (memoryview(address.encode('utf-8')), memoryview(data.encode('utf-8'))) )
    conn.commit()
    time.sleep(1)
